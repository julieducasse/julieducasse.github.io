...
// Old code
const renderBooksGender = (booksToRender) => {
    var books_by_gender = d3.nest()
        .key(function (d) { return d.Gender; })
        .entries(booksToRender)

    const genderScale = d3.scaleBand()
        .domain(booksToRender.map(function (d) { return d.Gender; }))
        .range([30, 120]);

    var genderGroups = svgPlot.selectAll(".genderGroups")
        .data(books_by_gender)
        .enter()
        .append("g")
        .attr("class", "gender") // + genderScale.bandwidth() / 2
        .attr("transform", d => `translate(0, ${genderScale(d.key) + genderScale.bandwidth() / 2})`);

    console.log(Object.keys(books_by_gender[1].values).length)

    var runningWidth = 0;
    var gender_books = genderGroups.selectAll("rect")
        .data(function (d) {
            console.log(d.key)
            return d.values
        })
        ...
        .attr("transform", function (d, i) {
            ...
            // var angle =  i==d3. ? -4 : 2 + Math.random();   
            var angle = i == 0 ? 10 : 2 + Math.random();
            return `rotate(${angle}, ${xRot},  ${yRot} )`
        })
        .on("mouseover", function (d) {
            showTooltip(divTooltip, d3.event.pageX, d3.event.pageY, d);
            // d3.select(this).call(updateBook, true);
        })
        .on("mouseout", function (d) {
            hideTooltip(divTooltip, d);
            // d3.select(this).call(updateBook, false);
        });

    // Horizontal shelves
    for (const entry of books_by_gender.entries()) {
        var lang = entry[1].key
        svgPlot.append("line")
            .attr("x1", 0)
            .attr("y1", genderScale(lang) + genderScale.bandwidth() / 2)
            .attr("x2", runningWidth + 10)
            .attr("y2", genderScale(lang) + genderScale.bandwidth() / 2)
            .attr("stroke-width", 2)
            .attr("stroke", "black");
...
const renderBooksAuthor = (booksToRender) => {
    var books_by_author = d3.nest()
    ...
    const authorScale = d3.scaleBand()
        .domain(books_by_author.map(function (d) { return d.key; }))
        .range([0, widthBookcase]);

    var authorGroups = svgPlot.selectAll(".authorGroups")
        .data(books_by_author)
        .enter()
        .append("g")
        .attr("class", "author") // + genderScale.bandwidth() / 2
        .attr("transform", d => `translate(${authorScale(d.key) + authorScale.bandwidth() / 2}, 0)`);

    var runningWidth = 0;
    var author_books = authorGroups.selectAll("rect")
        .data(function (d) {
            return d.values
        })
        .enter()
        ...
        .attr("x", 0)
        .attr("y", (d, i) => {
            if (i == 0) runningWidth = 0;
            var pos = heightBookcase - runningWidth - d.Width;
            runningWidth += (d.Width + 1);
            return pos;
        })
        .attr("stroke", "black")
